...
- **Contexto e Motivação:** O projeto iniciou com código exploratório concentrado em um único notebook (`pa1.ipynb`). Para permitir colaboração simultânea e reprodução limpa via terminal com `uv`, era necessário desacoplar responsabilidades em módulos Python (`pa1/`).
- **Como a IA auxiliou:**
...
- **Como a IA auxiliou:**
  ...
  - Divisão de responsabilidades entre Bruno e Elisa para desenvolvimento sem bloqueios ou dependências mútuas, com Bruno focado em dados, baseline semântica e mosaico; Elisa focada na geração do target de 3 classes, perdas focal e decodificação watershed.
- **Validação / Decisões Humanas:**
...
## Episódio 3: Geração do Target 3 Classes e Perda Focal

- **Data:** 06/09/2026
- **Ferramenta:** Antigravity / LLM
- **Contexto e Motivação:** Implementar a transformação das máscaras individuais do DSB2018 em mapa de 3 classes (fundo, interior, fronteira) e definir a função de perda adequada para o desbalanceamento extremo da classe fronteira.
- **Como a IA auxiliou:**
  - Esboço da lógica de dilatação das máscaras individuais para gerar a linha de fronteira (função `generate_3class_target` em `pa1/data/targets.py`).
  - Sugestão do uso de Focal Loss multiclasse com pesos de classe para compensar a minoria da classe fronteira (~1% dos pixels).
  - Implementação de `MulticlassFocalLoss` em `pa1/losses/segmentation.py`.
- **Validação / Decisões Humanas:**
  - Verificação manual de que a espessura da fronteira era consistente para núcleos de tamanhos variados.
  - Ajuste do peso da classe fronteira de 5 para 8 após análise dos primeiros resultados de treino.
...
- **Validação / Decisões Humanas:**
  - Teste manual do `inferencia.ipynb` com imagem qualquer para garantir que roda sem retreinar e produz resultados coerentes.
  - Revisão final do README.md seguindo a ordem do enunciado (ambiente → download → treino → avaliação → detalhamento por parte).
  - Reconstrução do `parte5_falhas_rf.ipynb` após corrupção de JSON.
