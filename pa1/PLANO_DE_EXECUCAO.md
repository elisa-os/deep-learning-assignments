...
│   └── watershed.py            # [✅ Concluído] Decodificação watershed com marcadores de interior e bacias (Passo 3)
├── metrics/
...
### Passo 1: Ingestão, Pré-processamento e Estratificação do DSB2018 — `[✅ Concluído]`
*Objetivo: Estruturar o carregamento do dataset real com particionamento estratificado confiável.*

- **O que fazer:**
  - `[✅ Concluído]` Download e extração das pastas de `stage1_train` (já presentes localmente em `pa1/data/stage1_train/` com 670 imagens e suas respectivas máscaras).
  - `[✅ Concluído]` Leitura das pastas `<image_id>/images/*.png` e consolidação de todas as anotações individuais `<image_id>/masks/*.png` em máscaras unificadas (semântica e instâncias $1 \dots N$).
  - `[✅ Concluído]` Classificação automática das imagens por modalidade biológica (fluorescência fundo escuro, campo claro fundo claro, e histologia H&E colorida) por meio de estatísticas de cor e histograma.
  - `[✅ Concluído]` Split estratificado: dividir em **70% Treino**, **15% Validação** e **15% Teste**, preservando a proporção de cada modalidade.
  - `[✅ Concluído]` Implementar `Dataset` e `DataLoader` PyTorch com augmentations (`albumentations`: RandomCrop 256x256, flips, rotações).
- **Onde fazer:**
  - `pa1/data/dsb2018.py` `[✅ Implementado]`
  - `pa1/main.py` `[✅ Implementado — bloco de carregamento de dados reais]`
  - `config.yaml` `[✅ Implementado — chave data.data_dir configurada]`
- **Como fazer:**
  1. Ler os diretórios `<image_id>/images/*.png` e consolidar todas as anotações individuais em `<image_id>/masks/*.png` em uma única máscara com IDs $1 \dots N$. `[✅ Feito]`
  2. **Estratificação por modalidade:** Extrair média e desvio padrão dos canais de cor e intensidade de fundo para agrupar as amostras em 3 grupos:
     ...
     - Histologia corada / H&E (coloração púrpura/rosada). `[✅ Feito]`
  3. Dividir os dados em **70% Treino**, **15% Validação** e **15% Teste**, preservando a proporção exata de cada modalidade em cada conjunto. `[✅ Feito — 468/101/101 com 546/19/105 por modalidade]`
  4. Configurar data augmentation com `albumentations` (RandomCrop 256x256, HorizontalFlip, VerticalFlip, RandomRotate90) apenas para o treino. `[✅ Feito]`

---

### Passo 2: Geração de Targets de 3 Classes para Trilha A — `[✅ Concluído]`
*Objetivo: Formular a representação multiclasse que permite ao modelo aprender a separação física entre núcleos encostados.*

- **O que fazer:**
  - `[✅ Concluído]` Criar função para converter máscaras individuais de instâncias em mapa de 3 classes (0: Fundo, 1: Interior erodido, 2: Fronteira dilatada).
  - `[✅ Concluído]` Parametrizar a espessura da fronteira (2 a 3 pixels) assegurando que núcleos vizinhos tenham uma barreira contínua.
  - `[✅ Concluído]` Integrar a transformação diretamente no `Dataset` do DSB2018 para retornar o tensor `mask_3class` no batch (disponível via `make_3class_mask()` em `targets.py`).
- **Onde fazer:**
  - `pa1/data/targets.py` `[✅ Implementado]`
  - `pa1/data/dsb2018.py` `[✅ Pronto para integrar na saída do getitem — função disponível]`
- **Como fazer:**
...
- **O que fazer:**
  ...
  - `[✅ Concluído]` Implementar decodificação avançada Watershed com marcadores de interior (`watershed_to_instances` em `watershed.py`).
- **Onde fazer:**
  - `pa1/postprocessing/connected_components.py` `[✅ Implementado]`
  - `pa1/metrics/instance.py` `[✅ Implementado]`
  - `pa1/postprocessing/watershed.py` `[✅ Implementado]`
- **Como fazer:**
...
- **Onde fazer:**
  ...
  - Saídas salvas em `pa1/outputs/ablation/` `[⏳ A Gerar]`
- **Como fazer:**
...
- **Onde fazer:**
  ...
  - `AI_LOG.md` `[⏳ A Criar]`
  - `README.md` `[✅ Atualizado — comandos de execução documentados]`
  - `pa1/outputs/checkpoints/best_model.pt` `[⏳ A Salvar após treino]`
- **Como fazer:**
  1. **`inferencia.ipynb`:**
...
- **Prevenção de Arquivos Pesados no Git:**
  ...
  - Checkpoints pesados devem ser salvos em `pa1/outputs/checkpoints/` e versionados externamente ou ignorados, mantendo apenas o link ou o peso final estritamente necessário.
- **Reprodutibilidade:**
